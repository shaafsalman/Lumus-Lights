...
import { getCompanyName, company, getPromotionalMessage } from '../CompanyDetails';
...
const NavigationBar = () => {
  ...
  useEffect(() => {
    const promotionalMessage = getPromotionalMessage();
    if (!promotionalMessage) return; // Early return if there's no message
  ...
  return (
    <>
      {getPromotionalMessage() && isVisible && (
