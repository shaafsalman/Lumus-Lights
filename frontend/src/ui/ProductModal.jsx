...
>
  ...
  <ActionButton type="button" onClick={addSku} text="Add SKU" />
    ...
    {isAddingSku && (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-4 border p-4 rounded">
        {[
          { id: 'temp-sku', label: 'SKU', value: tempSku.sku, type: 'text' },
          { id: 'temp-price', label: 'Price', value: tempSku.price, type: 'number' },
          { id: 'temp-stock', label: 'Stock', value: tempSku.stock, type: 'number' },
          { id: 'temp-color', label: 'Color', value: tempSku.color, type: 'text' },
          { id: 'temp-size', label: 'Size', value: tempSku.size, type: 'text' },
        ].map((field) => (
          <Input
            key={field.id}
            id={field.id}
            ...
            label={field.label}
            value={field.value}
            onChange={(e) => setTempSku({ ...tempSku, [field.id.split('-')[1]]: e.target.value })}
          />
        ))}
  ...
  <ActionButton text="Cancel" onClick={() => setIsAddingSku(false)} type="button" />
</div>
